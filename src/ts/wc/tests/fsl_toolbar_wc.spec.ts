...
describe('<fsl-toolbar>', () => {
  ...
  it('drives the editor theme + features and the host layout', async () => {
    ...
    // feature toggle → editor.noLint flips true (button shows un-pressed)
    expect(byText(toolbar, '.tb', 'Lint').getAttribute('aria-pressed')).toBe('true');   // on by default
    byText(toolbar, '.tb', 'Lint').click();
    await toolbar.updateComplete;
    expect(editor.noLint).toBe(true);
    expect(byText(toolbar, '.tb', 'Lint').getAttribute('aria-pressed')).toBe('false');

    // exercise the remaining control arrows (Light theme, Complete feature)
    q(toolbar, '[title="Light"]').click();
    await toolbar.updateComplete;
    expect(editor.theme).toBe('light');
    byText(toolbar, '.tb', 'Complete').click();
    await toolbar.updateComplete;
  ...
  it('renders inert controls when standalone (no host / editor)', async () => {
    ...
    expect(byText(toolbar, '.tb', 'Chips').getAttribute('aria-pressed')).toBe('true');   // editor-absent → on

    // every control click is a no-op (must not throw)
    q(toolbar, '[title="Dark"]').click();
    byText(toolbar, '.tb', 'Chips').click();
    q(toolbar, '[aria-haspopup="true"]').click();
    await toolbar.updateComplete;
