...
const isfinite_ = (x: number): boolean =>
  Number.isFinite(x);
...
const log = (x: number, base: number = Math.E): number => {
  if (x <= 0) {
...
const degrees = (rad: number): number =>
  rad * (180 / Math.PI);
...
const require_safe_int = (x: number, label: string): number => {
...
const require_uint_of_width = (x: number, width: number): number => {
...
const popcount = (x: number, width: number = 32): number => {
...
const clz = (x: number, width: number = 32): number => {
...
const ctz = (x: number, width: number = 32): number => {
  const v = require_uint_of_width(x, width);
  if (v === 0) { return width; }
  let count = 0;
...
const require_nonempty = (xs: readonly number[], label: string): void => {
  if (xs.length === 0) {
...
const product = (xs: readonly number[]): number =>
...
const mode = (xs: readonly number[]): number => {
