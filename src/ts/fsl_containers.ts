 ...
 */
function require_scalar(key: unknown): Scalar {

  if (typeof key === 'string')                     { return key; }
  if (typeof key === 'number' && Number.isFinite(key)) { return key; }
  throw new Error(
    `fsl container key/member must be a finite number or string, got ${stringify_for_error(key)}`
  );
...
function set_remove(set: FslSet, member: Scalar): FslSet {
  ...
  if (!set.members.includes(member)) { return set; }
...
function map_entries<V>(map: FslMap<V>): Array<[Scalar, V]> {
  ...
  return map.keys.map((k, i) => [k, map.values[i] as V]);
...
function snapshot(value: unknown): unknown {
  ...
  if (is_plain_object(value)) {
    ...
    for (const k of Object.keys(value).sort()) { out[k] = snapshot(value[k]); }
...
function deep_clone<T>(value: T): T {
  ...
  if (is_container(value)) {
    ...
    if (value.kind === 'set') {
      return { kind: 'set', members: Object.freeze(value.members.slice()) } as unknown as T;
    }
  ...
  if (is_plain_object(value)) {
    const out: Record<string, unknown> = {};
    for (const k of Object.keys(value)) { out[k] = deep_clone(value[k]); }
