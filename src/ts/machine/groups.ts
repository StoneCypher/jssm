...
export function isIn<mDT>(m: Machine<mDT>, groupName: string): boolean {
...
export function groupsOf<mDT>(m: Machine<mDT>, state: StateType): Set<string> {
...
export function groups<mDT>(m: Machine<mDT>): string[] {
...
export function statesIn<mDT>(m: Machine<mDT>, groupName: string): Array<StateType> {
...
export function groups_by_depth<mDT>(m: Machine<mDT>, state: StateType): string[] {

  const containing: string[] = [ ...groupsOf(m, state) ];

  if (containing.length < 2) { return containing; }

  return containing.sort((ga: string, gb: string): number => {

    const da: number = membership_distance(m._group_registry, state, ga),
          db: number = membership_distance(m._group_registry, state, gb);
