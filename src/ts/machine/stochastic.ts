...
import type {
  JssmGenericState, JssmTransition,
 ...
 */
export function start_state_weights<mDT>(m: Machine<mDT>): Map<StateType, number> {
 ...
 */
export function sample_start_state<mDT>(m: Machine<mDT>): StateType {
  if (m._start_state_weights.size === 0) { return m._start_states.values().next().value as StateType; }
 ...
 */
export function probable_exits_for<mDT>(m: Machine<mDT>, whichState: StateType): Array<JssmTransition<StateType, mDT>> {

  const wstate: JssmGenericState = m._states.get(whichState);
  if (!(wstate)) { throw new JssmError(m, `No such state ${JSON.stringify(whichState)} in probable_exits_for`); }
  ...
  const emg: Map<StateType, number> = m._edge_map.get(whichState);
  ...
  for (const ws of wstate.to) {
    ...
    const edge: JssmTransition<StateType, mDT> = m._edges[ emg.get(ws) ];
    /* v8 ignore next */
    if (!edge) { continue; }
 ...
 *  @internal
 */
function assert_selectable_exit_pool<mDT>(m: Machine<mDT>, whichState: StateType, exits: Array<JssmTransition<StateType, mDT>>): void {
 ...
 */
export function probabilistic_transition<mDT>(m: Machine<mDT>): boolean {
  const exits: Array<JssmTransition<StateType, mDT>> = probable_exits_for(m, state(m));
  assert_selectable_exit_pool(m, state(m), exits);
  const selected: JssmTransition<StateType, mDT> = weighted_rand_select(exits, undefined, m._rng);
 ...
 */
export function probabilistic_walk<mDT>(m: Machine<mDT>, n: number): Array<StateType> {
  return [...seq(n)
    .map((): StateType => {
      const state_was: StateType = state(m);
 ...
 *  non-empty but all-zero-weight (StoneCypher/fsl#1248).
 */
export function probabilistic_histo_walk<mDT>(m: Machine<mDT>, n: number): Map<StateType, number> {
 ...
 *  @internal
 */
function stochastic_one_walk<mDT>(
  m         : Machine<mDT>,
  start     : StateType,
  max_steps : number,
 ...
 */
export function* stochastic_runs<mDT>(m: Machine<mDT>, opts: JssmStochasticOptions = {}): Generator<JssmStochasticRun> {

  if (opts.seed !== undefined) { set_rng_seed(m, opts.seed); }
  ...
  const runs      : number             = (mode === 'steady_state')
    ? 1
    : (opts.runs ?? editor_config(m)?.stochastic_run_count ?? STOCHASTIC_DEFAULT_RUNS);

  const weighted_start: boolean   = m._start_state_weights.size > 0;
  const fixed_start   : StateType = state(m);
 ...
 */
export function stochastic_summary<mDT>(m: Machine<mDT>, opts: JssmStochasticOptions = {}): JssmStochasticSummary {

  const mode       : JssmStochasticMode = opts.mode ?? 'montecarlo';
  const saved_seed : number             = m._rng_seed;

  if (opts.seed !== undefined) { set_rng_seed(m, opts.seed); }
  const effective_seed: number = m._rng_seed;
  ...
  try {
    const run_stream = stochastic_runs(m, { ...opts, mode });
 ...
 */
export function rng_seed<mDT>(m: Machine<mDT>): number {
  return m._rng_seed;
}
 ...
 */
export function set_rng_seed<mDT>(m: Machine<mDT>, to: number | undefined): void {
