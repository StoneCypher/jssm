...
import type { JssmTransition, JssmArrowKind, JssmGenericState } from '../jssm_types.js';
...
export function transition_impl<mDT>(m: Machine<mDT>, newStateOrAction: StateType, newData: mDT | undefined, wasForced: boolean, wasAction: boolean, dataProvided: boolean = newData !== undefined): boolean {
  ...
  if (wasForced) {
    ...
    if (edgeId !== undefined) {
      const edge: JssmTransition<StateType, mDT> = m._edges[edgeId];
      valid      = true;
  ...
  // the destination (#729).
  const fromState: StateType   = m._state;
  const fromStateId: number    = m._state_id;
  const oldData: mDT           = m._data;
  ...
  if (m._has_post_hooks) {
    ...
    if (m._pre_post_everything_hook !== undefined) {
      m._pre_post_everything_hook({ ...hook_args, hook_name: 'pre post everything' });
    }

    if (wasAction) {
      // 1. any action posthook
      if (m._post_any_action_hook !== undefined) { m._post_any_action_hook(hook_args); }
    ...
    if (m._post_everything_hook !== undefined) {
      m._post_everything_hook({ ...hook_args, hook_name: 'post everything' });
    }
  ...
  if (m._event_listener_count !== 0) {
    const newData_after: mDT = m._data;
    ...
    const new_state_rec: JssmGenericState = m._states.get(newState);
...
export function go<mDT>(m: Machine<mDT>, newState: StateType, newData?: mDT): boolean {
  return transition_impl(m, newState, newData, false, false, arguments.length >= 3);
}
...
export function force_transition<mDT>(m: Machine<mDT>, newState: StateType, newData?: mDT): boolean {
...
export function act<mDT>(m: Machine<mDT>, actionName: StateType, newData?: mDT): boolean {
...
export function override<mDT>(m: Machine<mDT>, newState: StateType, newData?: mDT): void {
  ...
  if (allows_override(m)) {
    ...
    if (m._states.has(newState)) {
      ...
      if (dataProvided) { m._data = newData; }
      fire(m, 'override', {
        from     : fromState,
        to       : newState,
        old_data : oldData,
...
export function fire_hook_rejection<mDT>(
  m         : Machine<mDT>,
  hook_name : string,
  fromState : StateType,
  newState  : StateType,
...
export function fire_boundary_actions<mDT>(m: Machine<mDT>, prev_state: StateType, next_state: StateType): void {
  ...
  // set arithmetic.
  if (m._group_hooks.size === 0 && m._state_hooks.size === 0) { return; }
  ...
  const prev_groups: ReadonlySet<string> = m._state_to_groups.get(prev_state) ?? empty_string_set;
  const next_groups: ReadonlySet<string> = m._state_to_groups.get(next_state) ?? empty_string_set;
  ...
  for (const group of prev_groups) {
    ...
    const label: string | undefined = m._group_hooks.get(group)?.onExit;
    if (label !== undefined) { labels.push(label); }
  }
  const prev_state_exit: string | undefined = m._state_hooks.get(prev_state)?.onExit;
  ...
  for (const group of next_groups) {
    ...
    const label: string | undefined = m._group_hooks.get(group)?.onEnter;
    if (label !== undefined) { labels.push(label); }
  }
  const next_state_enter: string | undefined = m._state_hooks.get(next_state)?.onEnter;
