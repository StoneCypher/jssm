 ...
 *  @internal
 */
export const DEFAULT_TIME_SOURCE          = (): number => Date.now();
export const DEFAULT_TIMEOUT_SOURCE       = (f: () => void, a: number): number => {
...
export const DEFAULT_CLEAR_TIMEOUT_SOURCE = (h: number): void => clearTimeout(h);
 ...
 */
export function set_state_timeout<mDT>(m: Machine<mDT>, next_state: StateType, after_time: number): void {
  ...
  m._timeout_handle = m._timeout_source(
    ...
    () => {
      ...
      if (m._has_after_hooks) {
        ...
        if (ah !== undefined) { ah({ data: m._data, next_data: m._data }); }
        // a specific after hook firing implies the any-after hook fires too,
        // afterward; and it also fires alone (StoneCypher/fsl#1299)
        if (m._after_any_hook !== undefined) { m._after_any_hook({ data: m._data, next_data: m._data }); }
      }
 ...
 */
export function clear_state_timeout<mDT>(m: Machine<mDT>): void {
 ...
 *  @see current_state_timeout
 */
export function state_timeout_for<mDT>(m: Machine<mDT>, which_state: StateType): [StateType, number] | undefined {
 ...
 */
export function current_state_timeout<mDT>(m: Machine<mDT>): [StateType, number] | undefined {
 ...
 *  @see set_state_timeout
 */
export function auto_set_state_timeout<mDT>(m: Machine<mDT>): void {
  ...
  // short-circuits that.
  if (m._after_mapping.size === 0) { return; }
