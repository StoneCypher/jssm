...
describe('codegen', () => {
  ...
  it('reports undecided when the budget is exceeded', () => {
    // A 1ms budget against a real compile reliably trips the post-extract
    // check; if it somehow finished in under 1ms the assertion below would
    // tell us, but jssm compilation is far slower than a single millisecond.
    expect(() => codegen(turnstile, { target: 'native:javascript', budgetMs: 1 }))
