...
describe('val: accessors', () => {
  ...
  test('val on an unknown val throws', () => {
    const m = jssm.from('val n : int default 0; x -> y;');
    expect( () => m.val('ghost') ).toThrow(/No such val "ghost"/);
  });

  test('an unbounded int val_type omits lo and hi', () => {
    const m = jssm.from('val n : int default 0; x -> y;');
    expect( m.val_type('n') ).toEqual({ kind: 'int' });
  });

  test('a boolean val_type reads back', () => {
    const m = jssm.from('val ok : boolean default true; x -> y;');
    expect( m.val_type('ok') ).toEqual({ kind: 'boolean' });
  });

  test('a string val_type reads back', () => {
    const m = jssm.from('val s : string default "x"; x -> y;');
    expect( m.val_type('s') ).toEqual({ kind: 'string' });
  });

  test('an enum val_type lists its members', () => {
    const m = jssm.from('val t : enum(a, b, c) default a; x -> y;');
    expect( m.val_type('t') ).toEqual({ kind: 'enum', members: ['a', 'b', 'c'] });
  });
