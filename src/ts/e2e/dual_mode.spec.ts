...
async function host_fsl (page: Page): Promise<string> {
  return page.evaluate(() => (document.getElementById('host') as unknown as { fsl: string }).fsl);
}
...
test('dual-mode: editor seeds from host, edits write back, machine rebuilds at start state', async ({ page }) => {
  ...
  const before = await page.evaluate(() => {
    const host = document.getElementById('host') as unknown as { machine: { states(): string[] }, state(): string };
    return { states: host.machine.states(), state: host.state() };
  });
  expect(before.states.sort()).toEqual(['a', 'b']);
  ...
  await page.evaluate(() => (document.getElementById('host') as unknown as { transition(s: string): boolean }).transition('b'));
  const walked = await page.evaluate(() => (document.getElementById('host') as unknown as { state(): string }).state());
  ...
  expect(after.states.sort()).toEqual(['a', 'b', 'c']);
