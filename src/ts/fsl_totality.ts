...
type CycleResult<N> =
  | { acyclic : true,  cycle ? : undefined }
  | { acyclic : false, cycle   : N[]        };
...
function validate_graph<N>(graph: TotalityGraph<N>): void {
...
function build_adjacency<N>(graph: TotalityGraph<N>): Map<N, N[]> {

  const adjacency: Map<N, N[]> = new Map<N, N[]>();
...
function find_cycle<N>(graph: TotalityGraph<N>): CycleResult<N> {

  validate_graph(graph);

  const adjacency: Map<N, N[]> = build_adjacency(graph);

  const WHITE = 0,  // unvisited
        GREY  = 1,  // on the current DFS stack
        BLACK = 2;  // fully explored, no cycle through it

  const colour: Map<N, number> = new Map<N, number>();
  for (const node of graph.nodes) {
    colour.set(node, WHITE);
  }

  // explicit stack of (node, index-of-next-successor-to-visit) frames, plus a
  // parallel path of the grey nodes, so a cycle can be sliced out directly
  type Frame = { node: N, next: number };
    ...
    while (stack.length > 0) {
      ...
      if (frame.next < succs.length) {
        ...
        if (child_colour === GREY) {
          ...
          const start: number = path.indexOf(child);
...
function is_acyclic<N>(graph: TotalityGraph<N>): boolean {
...
function microstep_cascade<N>(
  graph : TotalityGraph<N>,
  start : N,
  bound : number = DEFAULT_MICROSTEP_BOUND
): CascadeResult<N> {
  ...
  const path    : N[]      = [ start ];
  const visited : Set<N>   = new Set<N>([ start ]);
...
function analyze_termination<N>(
  graph : TotalityGraph<N>,
  bound : number = DEFAULT_MICROSTEP_BOUND
):
  | { terminates : true }
  | { terminates : false, reason : 'cyclic',    cycle  : N[] }
  | { terminates : false, reason : 'unsettled', start  : N, cascade : CascadeResult<N> }
{

  const cycle_result: CycleResult<N> = find_cycle(graph);
  ...
  for (const node of graph.nodes) {
    const cascade: CascadeResult<N> = microstep_cascade(graph, node, bound);
