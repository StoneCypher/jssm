...
function is_weighted_list(x: unknown): x is JssmWeightedList {
  return (typeof x === 'object') && (x !== null) && ((x as { key?: unknown }).key === 'weighted_list');
}
...
function list_shares(list: Array<string> | JssmWeightedList): Array<{ name: string, share: number }> {
  ...
  const weighted  : Array<JssmWeightedListMember> = list.members.filter(m => m.weight !== undefined),
        unweighted: number                        = list.members.length - weighted.length;
...
function apply_list_share<StateType, mDT>(
  edge        : JssmTransition<StateType, mDT>,
  share       : number,
  is_list     : boolean,
  has_weights : boolean
  ...
  if (!is_list) { return; }
  if (edge.probability !== undefined) { edge.probability *= share; }
  else if (has_weights)               { edge.share = share; }
}
...
function compile_rule_transition_step<StateType, mDT>(
  ...
  const from_is_list    : boolean = Array.isArray(from) || is_weighted_list(from),
        to_is_list      : boolean = Array.isArray(to)   || is_weighted_list(to),
        from_has_weights: boolean = is_weighted_list(from),
        to_has_weights  : boolean = is_weighted_list(to);

  const from_shares: Array<{ name: StateType, share: number }> = from_is_list
    ? list_shares(from as unknown as Array<string> | JssmWeightedList).map(s => ({ name: s.name as unknown as StateType, share: s.share }))
    : [ { name: from, share: 1 } ];

  const to_shares: Array<{ name: StateType, share: number }> = to_is_list
    ? list_shares(to as unknown as Array<string> | JssmWeightedList).map(s => ({ name: s.name as unknown as StateType, share: s.share }))
    : [ { name: to, share: 1 } ];
  ...
  for (const { name: f, share: f_share } of from_shares) {
    for (const { name: t, share: t_share } of to_shares) {
      ...
      if (right.kind !== 'none') { apply_list_share(right, t_share, to_is_list, to_has_weights); acc.push(right); }
...
function compile<StateType, mDT>(tree: JssmParseTree<StateType, mDT>): JssmGenericConfig<StateType, mDT> {
  ...
  const raw_starts   : Array<StateType | JssmWeightedList> = results.start_states,
        weighted_start: JssmWeightedList | undefined =
          (raw_starts.length === 1 && is_weighted_list(raw_starts[0])) ? raw_starts[0] : undefined;

  const start_names: Array<StateType> = weighted_start
    ? weighted_start.members.map(m => m.name as unknown as StateType)
    : raw_starts as Array<StateType>;
  ...
  const result_cfg: JssmGenericConfig<StateType, mDT> = {
    ...
    state_property : [],
    ...(weighted_start && { start_state_weights: list_shares(weighted_start).map(s => ({ name: s.name as unknown as StateType, share: s.share })) })
  };
