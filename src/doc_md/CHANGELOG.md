...
582 merges; 415 releases; Changelogging the last 10 commits; Full changelog at [CHANGELOG.long.md](CHANGELOG.long.md)
...
Author: `John Haugeland <stonecypher@gmail.com>`
  ...
  * build(release): full release build for the 6.0.0-alpha.12 publish
  * Regenerates every tracked artifact via npm run build: root dist, the three member dists (committed for the first time -- the release job packs tracked files with no rebuild, so an untracked member dist ships an empty tarball), rolled-up typings, perf chart, site, and README.
  * README gains a 'Trying the v6 alpha' section: the v6 line publishes under the alpha dist-tag, so npm install jssm@alpha is the only way to get it and a plain npm install is unaffected.
  * The changelog step is NOT included -- better_git_changelog spawns 'git log --reflog' with node's 1MB default maxBuffer, and this branch's log output is 1.10MB. Fixing that upstream is the next step; CHANGELOG.md is unchanged from its committed state meanwhile.
...
## [Untagged] - Jul 25, 2026 10:11:20 AM

Commit [14f102dc71c3f53523d1c00c6259160d81105225](https://github.com/StoneCypher/jssm/commit/14f102dc71c3f53523d1c00c6259160d81105225)

Author: `John Haugeland <stonecypher@gmail.com>`

  * fix(release): publish prereleases under their own dist-tag, never latest
  * npm does not infer a dist-tag from a version's prerelease identifier: a bare 'npm publish' of 6.0.0-alpha.12 moves 'latest' to the alpha, so every 'npm install jssm' in the world resolves to a prerelease. publish_workspaces published bare, so landing v6 as an alpha would have done exactly that.
  * distTagFor derives the channel from the version -- 'alpha' for 6.0.0-alpha.12, 'latest' only for a full release -- and buildPublishArgs now requires an explicit tag rather than falling through to npm's default. One tag is computed per run from the lockstep version, so the root and its three members can never land on different channels. A numeric-only prerelease (6.0.0-12) throws rather than silently taking latest.
  * Verified by --dry-run: all four packages report 'Publishing with tag alpha'.
  * Also updates two specs that pinned counts changed by the size-chart work: the repo timeline now carries 32 repos (highlightjs/highlightjs-fsl), and collect_package_sizes tracks the five retired ecosystem packages.
...
## [Untagged] - Jul 25, 2026 8:14:51 AM

Commit [3e98c20e3ad6c8363587d1d9988d048d1f9a3630](https://github.com/StoneCypher/jssm/commit/3e98c20e3ad6c8363587d1d9988d048d1f9a3630)
...
<a name="v6__0__0-alpha__12" />

## [v6.0.0-alpha.12] - Jul 24, 2026 8:03:48 PM

Commit [81a0635f6cb4e91cc62089e8fa383cd4dc9caa9e](https://github.com/StoneCypher/jssm/commit/81a0635f6cb4e91cc62089e8fa383cd4dc9caa9e)

Author: `John Haugeland <stonecypher@gmail.com>`

  * ci(package-sizes): refresh the ecosystem repo timeline onto perf_results
  * The size chart draws non-npm repos as zero-mass lifespan rails, which needs their dates to travel with the archives. continue-on-error: a rate-limited lookup must not lose a night of size collection, and the chart renders fine without rails.
...
## [Untagged] - Jul 24, 2026 8:02:47 PM

Commit [01e7f2da0534101a92fd2ce78ef6592777fec28e](https://github.com/StoneCypher/jssm/commit/01e7f2da0534101a92fd2ce78ef6592777fec28e)

Author: `John Haugeland <stonecypher@gmail.com>`

  * build: wire size_chart into the release pipeline and collect the retired ecosystem packages
  * size_chart joins stage 9 beside the other docs/ writers, after site. collect_package_sizes now tracks the five retired ecosystem packages (jssm-viz-cli, jssm-viz-demo, codemirror-lang-fsl, require_jssm, fsl) as well as the live monorepo -- their archives never change again, but the mass-flow chart cannot draw a retirement whose bytes it never measured.
...
## [Untagged] - Jul 24, 2026 8:02:41 PM

Commit [4a1d386362d5423035f0d73296327d9b7ebc4a94](https://github.com/StoneCypher/jssm/commit/4a1d386362d5423035f0d73296327d9b7ebc4a94)

Author: `John Haugeland <stonecypher@gmail.com>`

  * feat(scripts): promote the package-size chart to src/scripts with a conservation build gate
  * The fsl#1965 chart moves out of the build/ prototype into three durable modules: flow_model.cjs (the pure Sankey mass-flow model), size_families.cjs (the file-family classifier), and make_size_chart.cjs (the generator). Both pure modules carry a node-only marker so the generator can inline the exact source the specs cover -- one copy, one proof -- and the generator throws if a marker ever goes missing.
  * Data comes from the perf_results branch the same way make_perf_chart reads its runs, with --from <dir> for local iteration. Output is docs/size_chart.html, which is gitignored, so a 600KB page regenerated nightly cannot churn the repo. Deterministic throughout: every timestamp comes from the data, never the wall clock. Degrades gracefully to exit 0 when the branch is unreachable, so offline builds still pass.
  * Conservation is now a build gate: the generator exits non-zero when the mass-flow graph fails to balance at any node, so a chart that misrepresents its own data cannot publish. It proves the routing is consistent with the measured masses; it cannot prove the archive was collected correctly.
  * 54 specs across the two pure modules.
...
## [Untagged] - Jul 24, 2026 6:43:09 PM

Commit [61e755b789f66212aa46147e09d24513c1492e9b](https://github.com/StoneCypher/jssm/commit/61e755b789f66212aa46147e09d24513c1492e9b)

Author: `John Haugeland <stonecypher@gmail.com>`

  * feat(scripts): repo lifespan dates and cross-owner repos for the Sankey rails
  * build_repo_timeline now records created/lastPush per repo, which is what the mass-flow view draws a non-npm repo's zero-mass rail across. A CATEGORIES entry may be written owner/name; those are fetched individually since gh repo list only covers one owner, so work adopted upstream (highlightjs/highlightjs-fsl) stays in the ecosystem's story. Records alpha-highlightjs-fsl -> highlightjs/highlightjs-fsl as a supersession, dated two days apart.
...
## [Untagged] - Jul 24, 2026 6:20:27 PM

Commit [ab2fdfb1c4fd4e10384356699b400a3ab8a0c10b](https://github.com/StoneCypher/jssm/commit/ab2fdfb1c4fd4e10384356699b400a3ab8a0c10b)

Author: `John Haugeland <stonecypher@gmail.com>`

  * docs(notes): Sankey mass-flow design for the package-size chart (fsl#1965)
...
## [Untagged] - Jul 24, 2026 1:48:09 PM

Commit [1b222544ce656eb421407c4c40321e54c67ac5e5](https://github.com/StoneCypher/jssm/commit/1b222544ce656eb421407c4c40321e54c67ac5e5)

Author: `John Haugeland <stonecypher@gmail.com>`

  * build(ci): published-bloat gate + record the ESM-only format split
  * Adds src/buildjs/verify_no_published_bloat.cjs, a release gate that runs
`npm pack --dry-run --json` for the root package and every packages/*
member and fails the build if any shipped path matches a denylist:
.nonmin. bundles, .map source maps, .rpt2_cache/ build caches, and
leaked node_modules/ trees.
  * This is the permanent form of "watch out for mistakes like 5.32.10" --
that release leaked *.nonmin.js at ~3.7M each, and 5.112.3 leaked
jssm_viz.*.nonmin.cjs at ~4M each; both were found only later, by size
archaeology. The pure logic (manifest parse + denylist match) sits
behind an injectable exec seam and carries 9 unit tests. Wired into the
verify-version-bump job, which release depends on, so a bloated tree
cannot publish.
  * Also records the accepted esm-only-main-package break in
v6_breaking_changes.json: the main package drops to a single ESM build
while es5 and iife relocate to compatibility packages (iife also serving
from the CDN), with the @fsl/statemachine-js naming destination noted.
  * Claude-Session: https://claude.ai/code/session_01GRhYnMn9uzb2kfHxRhTJ59
...
## [Untagged] - Jul 20, 2026 3:10:31 PM

Commit [b7762fac3abffbb63255fb6c2e5413d401c1ceda](https://github.com/StoneCypher/jssm/commit/b7762fac3abffbb63255fb6c2e5413d401c1ceda)

Author: `John Haugeland <stonecypher@gmail.com>`

  * perf: seed ecosystem-repo timeline dataset (31 repos, 8 categories)
  * Curated seed for the fsl#1965 alluvial/Sankey addendum, produced by
src/scripts/build_repo_timeline.cjs: repo, maintainer category (verbatim),
asides, and the supersession graph (6 edges resolve to jssm).