...
<dl>
  ...
  <dd>
    ...
    On a traffic light, we probably have <tt>next</tt>, <tt>turn on</tt>, and
    <tt>turn off</tt>. Actions are sometimes called <syn>tasks</syn>,
    <syn>raise</syn>, <syn>signal</syn>, <syn>event</syn>, or <syn>do</syn>.
    In some machines these are mandatory; in FSL they are optional.
  </dd>

  <dt><a name="data"></a>Data</dt>
  <dd>
    The difference between a Mealy and a Moore machine - data support means you
    can track more than just states.  In a vending machine, having no data means
    you need states for every valid sum of coin values; having data means you
    just track a number for what's already contained.  Some machines call this
  ...
  <dd>
    Support for general hooks means that you can establish a hook on general or
    global events, such as "any transition" or "any event."
  </dd>
  ...
  <dd>
    ...
    data passed to the posthook reflects the later configuration.
  </dd>
  ...
  <dd>
    ...
    streams.  From this worldview on finite state machines, the state alphabet
    <tt>Σ</tt> is your state list, the output alphabet is the same by default
    but could be changed by your hooks, the set of states <tt>S</tt> is just
    the states you've defined, the set of transformations <tt>𝛿</tt> accepts
    an input symbol (from the tape) if Moore and also some <syn>data</syn> if
    Mealy, which are your transitions, and the two streams are the input tape
    and the output tape.  If you'd like to write a FSM as an acceptor or a
    validator, typically you would use these tape facilities.  These tend to be
    found in parsing, iteration, and utility oriented machines.  If you are only
    using the input tape and a halting state (by example, a machine that checks
    if the input is a number,) you create an <syn>acceptor</syn>; if you use the
    output tape to produce a transformed set of symbols (by example, an
    upper-casing machine,) you have instead made a <syn>transducer</syn>.
    Support for tape is quite rare, despite being high value.
  </dd>
