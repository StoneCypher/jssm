 ...
 *
 *  @param has `{ set_hook, list_exits, edges_between, has_state }` booleans — each
 ...
 *  read-only `edges_between()` and `has_state()` columns are included only when
 *  the library exposes them.  Order is stable so trend tables line up across runs.
 *
 *  @param has `{ set_hook, list_exits, edges_between, has_state }` booleans.
 *  @returns The ordered list of operation tokens to register, each matching a key
 *           in `benchmark_scaling.cjs`'s case-factory map.
 *
 *  @example
 *  plannedCaseKinds({ edges_between: true, has_state: true })
 *  // => ['transition()', 'edges_between()', 'has_state()', 'construct()']
 *  @example
