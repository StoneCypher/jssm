#!/usr/bin/env node
// scripts/build.js — turn recipes/*.js into static cookbook pages.
//
...
function loadRecipes() {
  ...
  const files = fs.readdirSync(RECIPES).filter(f => f.endsWith('.js')).sort();
  const out = [];
...
function validate(slug, r) {
  for (const k of ['title', 'category', 'problem']) {
    if (!r[k] || typeof r[k] !== 'string') {
      throw new Error(`recipes/${slug}.js: missing required string field "${k}"`);
    }
  }
  if (slug.startsWith('test-')) {
    const t = parseTripleSlug(slug);
    if (!t) throw new Error(`recipes/${slug}.js: triple slug must be test-<runner>-<bundler>-<frontend>`);
    r.triple = r.triple || t;
  }
  if (r.blocks) {
    if (!Array.isArray(r.blocks)) throw new Error(`recipes/${slug}.js: blocks must be an array`);
    for (const [i, b] of r.blocks.entries()) {
      if (!b.kind || typeof b.kind !== 'string') throw new Error(`recipes/${slug}.js: blocks[${i}] missing kind`);
      if (typeof b.code !== 'string') throw new Error(`recipes/${slug}.js: blocks[${i}] missing code string`);
    }
  }
}
