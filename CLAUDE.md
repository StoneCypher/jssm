...
- **Even a docs-only / notes-only change forces a real npm release** of a new version. To avoid functionally-empty releases, **batch trivial changes** into a PR that carries real content rather than cutting a release per typo.
...
- **Pushes to `main`** additionally run the `build` matrix + `full-build`, then `release` (npm publish + tag) and `finish`.
