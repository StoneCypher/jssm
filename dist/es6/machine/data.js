 ...
 *
 */
export function validate_val_value(name, vtype, value, machine) {
    switch (vtype.kind) {
        case 'boolean': {
 ...
 *
 */
export function set_data(m, newData) {
 ...
 *
 */
export function strict_prop(m, name) {
    const bound_name = name_bind_prop_and_state(name, state(m));
    if (m._state_properties.has(bound_name)) {
        return m._state_properties.get(bound_name);
    }
 ...
 *
 */
export function props(m) {
    const ret = {};
    for (const p of known_props(m))
        ret[p] = prop(m, p);
    return ret;
}
 ...
 *
 */
export function known_prop(m, prop_name) {
 ...
 *
 */
export function set_val(m, name, value) {
    if (!m._val_keys.has(name)) {
        throw new JssmError(m, `No such val "${name}"`);
    }
 ...
 *
 */
export function known_val(m, name) {
 ...
 *
 */
export function val_type(m, name) {
    if (!m._val_keys.has(name)) {
        throw new JssmError(m, `No such val "${name}"`);
    }
