...
export function patch_state_fill(svg, state, fill) {
    let done = false;
    const out = svg.replace(NODE_GROUP_RE, (whole, body) => {
        if (done) {
            return whole;
        }
        const text = body.match(TEXT_RE);
        ...
        const patched_body = body.replace(SHAPE_FILL_RE, (_m, pre, _old, post) => `${pre}${fill}${post}`);
