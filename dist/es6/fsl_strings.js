 ...
 */
function resolve_index(index, len) {
 ...
 */
function resolve_bound(bound, len) {
 ...
 */
function pad_start(s, target, pad = ' ') {
 ...
 */
function build_padding(count, pad) {
 ...
 */
function from_utf8_bytes(bytes) {
 ...
 */
function setbyte(s, index, value) {
...
const GCB_RANGES = [
    ...
    [768, 879, GcbClass.Extend], // combining diacritical marks
    [1155, 1161, GcbClass.Extend],
    ...
    [2304, 2306, GcbClass.Extend], // Devanagari combining
    [2362, 2362, GcbClass.Extend],
    [2366, 2368, GcbClass.SpacingMark], // Devanagari spacing marks (sample)
    [2369, 2376, GcbClass.Extend],
    [4352, 4447, GcbClass.L], // Hangul leading jamo
    [4448, 4519, GcbClass.V], // Hangul vowel jamo
    [4520, 4607, GcbClass.T], // Hangul trailing jamo
    [8203, 8203, GcbClass.Control], // zero-width space
    [8205, 8205, GcbClass.ZWJ], // zero-width joiner
    [8288, 8292, GcbClass.Control],
    [9757, 9757, GcbClass.Extended_Pictographic],
    [9728, 9983, GcbClass.Extended_Pictographic],
    [9984, 10175, GcbClass.Extended_Pictographic],
    [44032, 55203, GcbClass.LVT], // Hangul syllable block (refined below)
    [65024, 65039, GcbClass.Extend], // variation selectors
    [65279, 65279, GcbClass.Control],
    [126976, 129791, GcbClass.Extended_Pictographic], // emoji planes
    [127462, 127487, GcbClass.Regional_Indicator], // regional indicators
    [917760, 917999, GcbClass.Extend] // variation selectors supplement
];
/**
 ...
 *  @see should_break
 */
const GCB_BREAK_AROUND = new Set([GcbClass.Control, GcbClass.CR, GcbClass.LF]);
/**
 ...
 *  @see should_break
 */
const GCB_AFTER_L = new Set([GcbClass.L, GcbClass.V, GcbClass.LV, GcbClass.LVT]);
/**
 ...
 */
function should_break(left, right, ri_run, emoji_zwj) {
    // GB3: CR × LF — never break inside a CRLF pair.
    if (left === GcbClass.CR && right === GcbClass.LF) {
        return false;
    }
    // GB4 / GB5: always break around Control / CR / LF (except the GB3 pair).
    if (GCB_BREAK_AROUND.has(left)) {
        return true;
    }
    if (GCB_BREAK_AROUND.has(right)) {
        return true;
    }
    // GB6 / GB7 / GB8: Hangul jamo sequences.
    if (left === GcbClass.L && GCB_AFTER_L.has(right)) {
        return false;
    }
    if ((left === GcbClass.LV || left === GcbClass.V) &&
        (right === GcbClass.V || right === GcbClass.T)) {
        return false;
    }
    if ((left === GcbClass.LVT || left === GcbClass.T) && right === GcbClass.T) {
        return false;
    }
    // GB9 / GB9a / GB9b: do not break before Extend / ZWJ / SpacingMark, nor after Prepend.
    if (right === GcbClass.Extend || right === GcbClass.ZWJ) {
        return false;
    }
    if (right === GcbClass.SpacingMark) {
        return false;
    }
    if (left === GcbClass.Prepend) {
        return false;
    }
    // GB11: within an emoji ZWJ sequence (pictographic Extend* ZWJ × pictographic).
    if (emoji_zwj && right === GcbClass.Extended_Pictographic) {
        return false;
    }
...
function to_graphemes(s) {
    ...
    for (let i = 1; i < cps.length; ++i) {
        ...
        if (right === GcbClass.Extended_Pictographic) {
            pict_open = true;
            emoji_zwj = false;
        }
        else if (right === GcbClass.Extend && pict_open) { /* stay open */
            emoji_zwj = false;
        }
        else if (right === GcbClass.ZWJ && pict_open) {
            emoji_zwj = true;
        }
        else {
            pict_open = false;
 ...
 */
function grapheme_slice(s, lo, hi) {
    const gs = to_graphemes(s);
...
// grapheme unit
gcb_class, should_break, to_graphemes, grapheme_length, grapheme_at, grapheme_slice, grapheme_reverse };
export { GcbClass };
