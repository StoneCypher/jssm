...
- **Viewer representation selector (folds in `fsl-output`):** the viewer pane shows the machine rendered as **Diagram / DOT / SVG / Config**; these are *representations of one thing*, not separate panels. Float a single **segmented-pill control** in a **corner** of the viewer pane (no header bar) — one rounded-endcap capsule, vertical dividers, the selected segment depressed (inset + accent fill), label inside each segment — e.g. `( Diagram | DOT | SVG | Config )`. The graphviz engine selector and a copy/download button ride as small contextual corner affordances (e.g. copy appears only on the source representations). **Orthogonal to arrangement** (the View menu): the same selector in split / stacked / single / tabbed (tabbed keeps `[Code | Viewer]` tabs; the Viewer tab carries the pill). **Shape vocabulary:** rounded connected segmented-pill = mutually-exclusive single-select (radio); the toolbar's square flat separable buttons = independent toggles. If two representations need to be seen side by side, promote DOT/SVG to toggleable panels instead (default is the pill — one representation at a time, keeping the diagram's space).
