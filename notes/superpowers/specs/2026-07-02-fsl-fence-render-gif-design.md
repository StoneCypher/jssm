...
- Internals: one **global color table** (median-cut quantization to ≤256 colors over the first frame's pixels, subsequent frames mapped nearest-neighbor), LZW compression with correct code-size resets and 255-byte sub-block packing, Graphics Control Extension per frame (delay, no transparency v1), Netscape 2.0 looping application extension, trailer.
