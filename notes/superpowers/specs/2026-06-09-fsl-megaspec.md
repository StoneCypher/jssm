...
**In-flight upgrade (`migrates from`).** Long-running machines are *mid-flight when v2 deploys*, carrying v1-schema snapshots — durable execution's hardest problem (Temporal's most cursed corner). FSL answers it structurally: a **`migrates from v1 { attempts := retries; tier := free; }`** block declares the typed lifting of old snapshots into the new val schema; the verifier checks lifted states are valid (in-domain, contracts hold) and refinement checking confirms migrations land in behaviorally-coherent configurations. Version gating remains the refusal path; `migrates from` is the affirmative one that makes refusal survivable. Claiming the durable-execution category obligates this feature.
...
**Location transparency (the distribution door).** Wiring is specified **location-transparently**: nothing in a system document assumes members share a process, so a channel can later bind to a **transport** (WebSocket, queue, shared memory) without the document changing. The determinism story then *adapts* rather than breaks — per-member tapes plus **causal merge** (Lamport ordering) give distributed replay. Partition/retry/at-least-once semantics are real future spec work (§28); this paragraph exists so §22's dispatch design never paints distribution out.
...
**Assume-guarantee composition — the real answer to state explosion.** Budgets postpone the product blow-up; compositional verification dissolves it: each member is verified once against its **interface contract** (assumptions on inputs, guarantees on outputs — the interface card's formal skeleton), and the system is checked by **composing contracts instead of state spaces**. The 500-customer park stops being a 500-fold product: one customer-contract proof plus a contract-level composition argument. The §27 session-types line becomes a feature here: a channel's protocol *is* a contract, so **wiring typechecks behaviorally from interfaces alone** — compatibility verdicts without ever building the product. Certificates extend piecewise: member certificates + a composition certificate = a system certificate.

**Flow contracts — noninterference, decidably.** `disallow flow pin -> log;` proves the value of `pin` can never influence what appears on the `log` channel — taint-product reachability, **decidable for finite machines**, through any chain of assigns, guards, or routes. This completes redaction's read-side analysis (§25 `--redact`: "nothing observes this field") into the full lattice ("this field never *reaches* there"). Guardrails constrain *actions*; flow contracts constrain *data* — together they are the security story: "prove the secret never reaches telemetry" as a checkbox, not a research project.

**SLO contracts.** PCTL in contract position: `invariant P>=0.999 [eventually served within 100 steps];` — a machine-checked service-level objective, proven by the Storm/PRISM backend at design time and monitored against the live tape by drift detection (§15) at runtime. The same property is the design-time proof and the production monitor.
...
| `walk` | seeded walks / statistical tier (§17) | statistical MC w/ confidence intervals, coverage-guided tape generation, full-surface fuzzing, adversarial counterexample search, **shrinking**; the stimulus *generator* feeding `run`/`test`/§26 vectors; **`--sweep binding=lo..hi:step`** runs the experiment design across factory bindings (the factory *is* the sweep generator; seed tree gives every cell reproducibility; Markov analytics summarize each) — discrete-event simulation / digital-twin workflows, with verifiability attached; **`--reach <state\|predicate>`** produces a stimulus tape reaching a named configuration (concolic/SMT-steered) — "show me a path to deadlock" as a debugging, teaching, and test-authoring tool |
...
| `synth` *(aspirational)* | derive a policy machine from properties | supervisory-control / safety-game solving (decidable for the safety fragment over finite alphabets): write the forbidden-sequence properties (in Dwyer patterns), get the **maximally-permissive guardrail machine**, correct by construction, certificate attached — "don't write the guardrail, derive it"; completes the ladder *`infer` learns from behavior · `check` judges against properties · `synth` derives from properties* |
...
**Verification certificates (proof-carrying machines).** `check --emit-certificate` produces a small, portable, sign-able record: **document `source_hash`, properties verified, checker version, verifiability tier, result** — an attestation that *this machine, at this hash, was proven*. CI verifies the certificate without re-running the (possibly expensive) check; an agent receiving a machine confirms what was proven, by what, against what, before composing with it. The proof-carrying-code idea, sized for state machines — in an ecosystem where agents exchange machines, **the certificate is the trust currency**, and it makes §25.1's "the verifier vouches for it" literal: an artifact, not a slogan. A certificate is invalidated by any source change (the hash) or checker change (the version); re-certification is one command. **Identity & provenance:** certificates are **signed** (sigstore-style keyless signing — bound to an OIDC identity: a CI workflow, a maintainer) and recorded in an append-only **transparency log**, making them attributable and tamper-evident. Without identity a certificate is self-asserted and `verify` would accept fabricated claims; with it the complete sentence is *"proven, by this checker, run by this identity, logged publicly."* Mostly integration (sigstore exists), not invention. **Refinement certificates** extend the idea to evolution: `diff --behavioral --emit-certificate` attests *"v2 refines v1 at these hashes"* — a dependency upgrades with a proof that nothing observable regressed, which is the difference between "I refactored it and the tests pass" and "here is the proof it's behaviorally compatible." **Free theorem:** if v2 refines v1, then **v1's regression tapes replay on v2** by the simulation map — a compatible upgrade keeps the entire recorded test suite for free.
...
- **Adapter conformance:** adapter codegen targets (xstate, boost::msm, …) don't merely *declare* feature subsets — the vector suite **runs through the adapter's actual runtime** on the declared subset, so "export to xstate" means *behaviorally certified on the features it claims*, and the FeatureComparison matrix is a tested contract rather than a table.
- **The toolchain fuzzes itself:** the GBNF grammar artifact, pointed back at our own parser and checker, generates millions of valid and mutated-invalid documents asserting parser totality, `format` idempotence, and verdict stability across checker backends. The toolchain that asks to be trusted with proofs is hardened by its own artifacts.
...
| identity & distribution ring (batch 11) | **certificate signing + transparency log** (sigstore keyless, OIDC-bound, append-only — certificates attributable + tamper-evident; unsigned = self-asserted theater); **`migrates from`** (typed snapshot lifting for in-flight upgrades — durable execution's hardest problem; verifier-checked, refinement-vetted); **location-transparent wiring** (the distribution door: transports later, no document changes; per-member tapes + causal merge = distributed replay; partitions = §28); **adapter conformance** (vector suite through adapter runtimes — the FeatureComparison matrix becomes a tested contract); **parameterized verification** (aspirational: for-all-N via cutoffs); **toolchain self-fuzzing** (the GBNF artifact pointed at our own parser; format idempotence + verdict stability under fuzz) |
| verifier-theory ring (batch 10) | **assume-guarantee composition** (§23: interface contracts as the unit of composition — compose contracts, not state spaces; session-typed wiring checks from interfaces alone; piecewise system certificates); **flow contracts** (`disallow flow a -> b;` — noninterference by taint-product reachability, decidable for finite; completes `--redact`'s read-side into the full lattice; guardrails=actions, flow=data); **`synth`** (aspirational: safety-game policy synthesis — derive the maximally-permissive guardrail from Dwyer-pattern properties; the infer/check/synth ladder complete); **SLO contracts** (PCTL in contract position — design-time proof + runtime drift monitor, same property); **`walk --reach`** (concolic witness generation); **CEGAR** (aspirational §28: the checker's third scaling rung); **free theorem** (refinement ⇒ old tapes replay on v2 — regression suites survive compatible upgrades) |
...
## 28. Open / deferred
Regex; locale-tailored normalization/casing; dense-time verification (#1361) + probabilistic timed automata (#1362); parallel regions (#1353); **semantic patches** (the dual of `fsl diff` — a state/transition/val-level patch format applied without string surgery; the door for multi-agent collaboration on one machine; `format` idempotence already makes textual editing safe, so this waits until agents are heavy concurrent authors); **hosting the verified-component registry** (the entry format is specced — §25.1 — running an index is operational and deferred); **inductive-invariant witnesses** (the strong form of `check --witness`; coverage facts ship first); **CEGAR** (counterexample-guided abstraction refinement — the third rung of the checker's scaling ladder after native-exhaustive and assume-guarantee-compositional: abstract val domains to intervals/predicates, refine exactly where a spurious counterexample lied); **parameterized verification** ("safe for ALL population sizes," not just ≤max — cutoff theorems / invisible invariants; the honest endpoint of the population story); **distribution semantics** (transports, partitions, at-least-once — the §22.4 location-transparency door's eventual contents). *(Now specced, not deferred: contract model #1355 → §10/§11; RTC/microstep #1345 → §12.)*
