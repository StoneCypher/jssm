...
| `inspect` | read-only repro-bundle / snapshot / tape reader | seed, FSL + machine versions, digest, event count, §15 compatibility verdict — see *why* a bundle is refused before trying it |
...
| `mcp` | MCP server (stdio) | exposes the verb set (parse/lint/check/run/walk/render/make…) as MCP tools for AI agents — the agent-facing twin of `lsp`, same internals, different protocol |
