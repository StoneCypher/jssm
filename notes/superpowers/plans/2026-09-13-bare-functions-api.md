...
- [ ] **Step 2: Bundle-size test** — `compat_bundle_size.spec.ts`, guarded like `published_files.spec.ts` with `describe.skipIf(!existsSync(resolve(root, 'dist/es6/compat.js')))`: use rollup's JS API (`import { rollup } from 'rollup'`) with a virtual entry plugin, `treeshake: true`, no minification, `@rollup/plugin-node-resolve` as the core config does, and assert what is true in 6.0 (controller ruling 2026-09-13 — the original `a.length < b.length` instruction was false by construction: `sm`/`from`/`create` all do `new Machine(...)`, and the class's delegates reference every family, so a factory-importing bundle is a superset of the compat bundle): bundle C = `export { state, transition } from '<root>/dist/es6/jssm.js'` contains NO `class Machine` and DOES contain the transition-core marker `_committing_transition`; bundle A = `export { sm, state } from '<root>/dist/es6/jssm.js'` and bundle B = `export { Machine } from '<root>/dist/es6/compat.js'` BOTH contain `class Machine` (a describe named "in 6.0 a factory import carries the compat class"); no size comparison between A and B is asserted, and no golden number either way. The 6.0 size win exists only for modules that import functions and receive a machine from elsewhere, and in full only when a later major drops the compat prototype from the default value. If the ci-lite legs lack `dist/es6`, the guard skips it; the release build runs it.
