...
- 2026-07-04 · **v16** · Public · 10 files (registry W12.1,2; guardrails W12.3; query-patches W12.4,5; infer-synth W12.6,7; corpus W12.8; distribution W12.9,10) — registry loop-first + behavioral search, mcp --policy guardrails, fleet query + semantic patches, infer/synth bookends, curriculum/eval corpus, machines.txt + distribution program. issue-twin W12.9=778. `npm run era_book` green (271 files). **Era 7 (v16) W-spine COMPLETE — ALL W-ITEMS DONE (W6.*–W12.* + all WP program topics).**

### MILESTONE 1: full W-item + program-topic spine seeded (271 items, all 12 versions)

### Tracker-row residue sweep (subagent-parallelized; main session committed)
Non-W-twin tracker rows, drafted by per-era Sonnet agents (voice-passed + gated + committed here):
- **e1 → v7–v10** · 121 files — charset/destructuring (v7); integer IDs/probabilistic origins/named lists (v8); error-object/strict-reads/assign-reset-lifecycle (v9); viz+themes+kinds, editor QoL + wc panels, CLI verbs, serialize/typegen/docgen, the perf lane, build-matrix, conformance/proof-corpus, FSL→Mermaid (v10 bulk).
- **e2 → v11** · 26 files — editor/viewer/remote-subgraph/tape-io/hooks/framework-wrappers.
- **e3 → v12** · 5 files — leanstral prover, semantic debugger, dense-time/PTA tiers, Playwright MBT.
- **e4 → v13** · 2 files — storage notation, serialize umbrella (authored inline; e4 is tiny).
- **e5 → v14** · 0 files — entirely jssm trust twins already covered by W10.*.
- **e6 → v15** · 2 files — libsir bindings, code-emission umbrella (bucket is mostly W11.3 twins).
- **e7 → v16** · 136 files — the distribution/ecosystem block (AI-distribution #831–869, MCP/skills registries, listings, docs/SEO, README+site, apps, gallery, integrations, Discord+auto-update fabric, Playwright announcer, self-hosting toolchain). The real fsl org-migration issue (Tfsl-223) landed at v15 and superseded the synthetic WP15.1 placeholder.

**FOLDING:** where several tracker rows are one deliverable across surfaces (e.g. the six error-object-field rows, hover labels, URL decoration, theme-coverage, node-matrix legs, charset surfaces, repl/lsp fsl+jssm twins), agents folded them into a single item and recorded the folded issue numbers in `cs`. So the register is COMPLETE in coverage even though the item count (562) is below the raw row count — folded rows are covered, just grouped.

### MILESTONE 2 (COMPLETE): register seeded end to end
**562 items across all 12 versions** — 5.x=32, v6=19, v7=46, v8=35, v9=32, v10=120, v11=64, v12=28, v13=17, v14=11, v15=13, v16=145. Plus the per-era Mermaid dependency diagrams wired into `era_book_html.cjs` (renders client-side; dashed = cross-era). `npm run era_book` green on every slice.

**Known cosmetic nit:** the split-authored files (e1/e7) use multi-line `src` arrays (from `JSON.stringify`), where hand-authored ones use inline arrays. Valid + renders identically; a one-time formatter pass could normalize if desired.
- 2026-07-04 · **v15** · Ubiquitous · 11 files (abi-migration W11.1 + WP15.1→superseded by Tfsl-223; per-host-runtimes W11.2; codegen-adapters W11.3; fleet-certification W11.4,9; typegen-manifests-wasm W11.5–8,10) — host ABI freeze + org migration, bringup kit, T2/T3 differential fleet CI, codegen/adapters, fleet typegen + manifests + WASM + publish. issue-twins W11.3=563, W11.4=815, W11.7=1172, W11.10=407. `npm run era_book` green (261 files). **Era 6 (v15) W-spine COMPLETE.**
- 2026-07-04 · **v14** · Trusted · 11 files (security-gate WP14.1; m2-signing W10.1–3; m5-verify W10.4; m6-receipts W10.5–8; ecosystem-plumbing W10.9,10) — sigstore keyless + transparency log + revocation, verify chain, receipt tiers (replayed/hosted/witnessed) + holder-key, fsl.lock. jssm twins #829(M5)/#830(M6). `npm run era_book` green (250 files). **Era 5 (v14) W-spine COMPLETE.**
- 2026-07-04 · **v13** · Durable · 15 files (process-gate WP13.1; persistence W9.1; effect-recording W9.2; migration-upgrade W9.3–5; flight-recorder W9.6–8; forensics W9.9–12; policy-decisions W9.13,14) — durable execution, recorded effects, live migration/canary, OTel+drift+calibrate, replay forensics. issue-twin W9.1=813. `npm run era_book` green (239 files). **Era 4 (v13) W-spine COMPLETE.**
